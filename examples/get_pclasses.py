...
try:
	pclasses = k.get_pclasses(
		type=None)  # Here we can define a specific type of PClass we want
					# e.g Klarna.PClass.Type.CAMPAIGN

	print pclasses
	for id, pclass in pclasses.items():
		print "%s\t%s" % (pclass.id, pclass.description)
except Exception, e:

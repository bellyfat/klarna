...
##
# 3. Invoke activate_part
#

# Here you specify the quantity of an article you wish to partially activate.
# artno must be the same as the one you used in add_article when you made 
# the add_transaction call.
...
try:
	...
	result = k.activate_part(invno)

	print "%s\t%s" % (result['url'], result.get('invno', 0))
except Exception, e:
	logger.debug("call activate_part failed", exc_info=True)
	sys.exit(1)
